...
const InvoiceList = () => {
    ...
    const fetchInvoices = async (pageNum, resetData = false) => {
        try {
            ...
            if (result.status === "success") {
                const newData = result.result.data.filter((elm) => elm.cusType != 'B');
                // console.log(newData);
    ...
    async function fetchInvoiceItems(invoiceId, cusType, silverRate, invoiceDate) {
        try {
            ...
            if (result.status == 'success') {
                ...
                const component = getBillPDF(cusType, mappedItems, silverRate, invoiceDate);

                if (!component) return; // stop execution
    ...
    function getBillPDF(cusType, updatedItems, silverRate, invoiceDate) {
        ...
        // console.log(isPExists);
        // console.log(isSExists);

        switch (cusType) {
            case 'R':
                return <BillPDF items={updatedItems} silverRate={silverRate} invoiceDate={invoiceDate} isPExists={isPExists} isSExists={isSExists} />
            case 'W':
                return <WholeSalerBillPDFA items={updatedItems} silverRate={silverRate} invoiceDate={invoiceDate} isPExists={isPExists} isSExists={isSExists} />
            default:
                console.log("Customer type neither 'Retail' nor 'WholeSale' ");
        ...
        <div className="flex gap-4">
            ...
            {showInvoice.show &&
                <ViewInvoices
                    ...
                    onUpdateSilverRate={(newRate) => {
                        setInvoices(prev => prev.map(inv => 
                            inv.invoiceNo === showInvoice.invId 
                                ? { ...inv, silverRate: newRate } 
                                : inv
                        ));
                        // Also update current active modal state just in case
                        setShowInvoice(prev => ({ ...prev, silverRate: newRate }));
                    }}
                />
            }
            ...
            <div className="p-5 pt-8 mb-2 mx-auto w-[80%]">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="bg-white rounded-lg shadow overflow-hidden">
                        {/* Table Header */}
                        <div className="grid grid-cols-[60px_1fr_1fr_1fr_1fr_1fr_1fr_1fr] gap-4 px-6 py-4 bg-gray-100 border-b border-gray-200 font-semibold text-gray-600 uppercase text-sm">
                        ...
                        <div className="divide-y divide-gray-200">
                            {invoices.length > 0 ? (
                                invoices.map((inv, idx) => (
                                    <div
                                        key={inv.invoiceNo}
                                        className="grid grid-cols-[60px_1fr_1fr_1fr_1fr_1fr_1fr_1fr] gap-4 px-6 py-3 hover:bg-gray-100 cursor-pointer items-center transition"
                                    >
